...
sub encode_offsetcommit_request {
    ...
    if ($is_v1) {
        ...
        $request->{len}         += 4;
        push( @data, $OffsetCommit_Request->{ GenerationId } );

        $request->{template}    .= q{s>};                                   # string length
        $request->{len}         += 2;
        _encode_string( $request, $OffsetCommit_Request->{ MemberId } );    # MemberId
    }
...
my $_decode_member_metadata_template = q{s>l>X[l]l>/(s>/a)l>/a};
                                            # s>            # Version
                                            # l>            # Topics array size
                                            # X[l]
                                            # l>/(
...
sub decode_joingroup_response {
    ...
    while( $Members_array_size-- ) {
        my $member_id               = $data[$i++];
        my $member_metadata_packed  = $data[$i++];

        my @member_metadata_comps = unpack( $_decode_member_metadata_template, $member_metadata_packed );

        my $member_metadata = {};
        my $j = 0;
        $member_metadata->{ Version }   = $member_metadata_comps[$j++];
        my $topics_array_size = $member_metadata_comps[$j++];
        while( $topics_array_size-- ) {
            push @{ $member_metadata->{ Topic } //= [] }, $member_metadata_comps[$j++];
        }
        $member_metadata->{ UserData }  = $member_metadata_comps[$j++];
